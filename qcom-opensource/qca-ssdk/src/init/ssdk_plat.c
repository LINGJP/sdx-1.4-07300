...
#ifdef BOARD_MDM_LE
#define MDM_LE_MDIO_BUS_NAME		"dwc_phy"
#define MDM_LE_MDIO_BUS_ID			0x1
#define MDM_LE_MDIO_PHY_ID			0x0

#define MDM_LE_NTN3_MDIO_BUS_NAME		"tc956xmac"
#define MDM_LE_NTN3_MDIO_BUS_ID			0x1
#define MDM_LE_NTN3_MDIO_PHY_ID			0x1c
#endif
...
static int miibus_mdm_get(const char *mdio_bus_name, int mdio_bus_id, int mdio_phy_id)
{
	...
	snprintf(phy_id_fmt, MII_BUS_ID_SIZE + 3, "%s:%02x", busid, mdio_phy_id);

	miidev = bus_find_device_by_name(&mdio_bus_type, NULL, phy_id_fmt);
	if (miidev) {
...
#ifdef BOARD_MDM_LE
#define AR8327_NUM_EXT_PORTS 5
void ssdk_port_link_event_receive(ssdk_port_status *ports_status);
static ssize_t ssdk_port_link_get(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	ssize_t count = 0;
	a_uint32_t port;
	ssdk_port_status ports_status[AR8327_NUM_PORTS] = {0};

	ssdk_port_link_event_receive(ports_status);
	for(port = 1; port <= AR8327_NUM_EXT_PORTS; port++) {
		count += snprintf(buf+count, (ssize_t)PAGE_SIZE, "[%d]:%-4s ", port,
						ports_status[port].port_link?"up":"down");
	}

	count += snprintf(buf+count, (ssize_t)PAGE_SIZE, "\n");
...
#ifdef BOARD_MDM_LE
static const struct device_attribute ssdk_port_link_attr =
	__ATTR(port_link, 0664, ssdk_port_link_get, NULL);
#endif
...
#ifdef BOARD_MDM_LE
	/* create /sys/ssdk/port_link file */
	ret = sysfs_create_file(ssdk_sys, &ssdk_port_link_attr.attr);
	if (ret) {
		printk("Failed to register SSDK port link file\n");
		goto CLEANUP_8;
	}
#endif
...
#ifdef BOARD_MDM_LE
CLEANUP_8:
	sysfs_remove_file(ssdk_sys, &ssdk_port_link_attr.attr);
#endif
