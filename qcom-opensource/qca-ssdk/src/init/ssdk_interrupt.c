 ...
 int qca_intr_init(struct qca_phy_priv *priv)
{
 ...
 int qca_intr_cleanup(struct qca_phy_priv *priv)
{
	SSDK_DEBUG("start to  cleanup the interrupt!\n");

	qca_phy_disable_intr(priv);
	qca_mac_disable_intr(priv);
	synchronize_irq(priv->link_interrupt_no);
	free_irq(priv->link_interrupt_no, priv);
	cancel_work_sync(&priv->intr_workqueue);
