...
#ifdef BOARD_MDM_LE
void qca_ar8327_rgmii0_pwr_src_sel_by_dts(struct qca_phy_priv *priv)
{
	struct device_node *emac_node = NULL;
	struct device_node *vreg_rgmii_io_pads = NULL;
	const __be32 *reg_rgmii_io_pads_voltage;
	a_uint32_t reg_rgmii_io_pads_voltage_val;
	a_int32_t len;
	a_uint32_t value = 0;

	emac_node = of_find_node_by_name(NULL, "qcom,emac");
	if (!emac_node) {
		SSDK_INFO("emac_node DT doesn't exist!\n");
		return;
	}

	vreg_rgmii_io_pads = of_parse_phandle(emac_node, "vreg_rgmii_io_pads-supply", 0);
	if (!vreg_rgmii_io_pads) {
		SSDK_INFO("vreg_rgmii_io_pads-supply DT doesn't exist!\n");
		return;
	}

	reg_rgmii_io_pads_voltage = of_get_property(vreg_rgmii_io_pads,
		       				"regulator-min-microvolt", &len);
	if (!reg_rgmii_io_pads_voltage) {
		SSDK_INFO("regulator-min-microvolt DT doesn't exist!\n");
		return;
	}

	reg_rgmii_io_pads_voltage_val = be32_to_cpup(reg_rgmii_io_pads_voltage);
	SSDK_INFO("IOMACRO pads voltage DT: %u uV\n", reg_rgmii_io_pads_voltage_val);

	value = priv->mii_read(priv->device_id, AR8327_REG_PAD_MAC_PWR_SEL);

	if (reg_rgmii_io_pads_voltage_val == 1800000) {
		SSDK_INFO("select 1.8v for rgmii0 power source\n");
		value |= AR8327_PAD_MAC_PWR_RGMII0_1_8V;
	} else {
		SSDK_INFO("select 1.5v for rgmii0 power source\n");
		value &= ~AR8327_PAD_MAC_PWR_RGMII0_1_8V;
	}
...
int qca_ar8327_hw_init(struct qca_phy_priv *priv)
{
	...
	paddr = of_get_property(np, "qcom,ar8327-initvals", &len);
	if (paddr) {
...
#ifdef BOARD_MDM_LE
	qca_ar8327_rgmii0_pwr_src_sel_by_dts(priv);
#endif
...
static int qca_link_polling_select(struct qca_phy_priv *priv)
{
	...
	if(of_property_read_bool(np,"qcom,link-polling-required"))
	{
		priv->link_polling_required = 1;
	
	} else {
		SSDK_INFO("qcom,link-polling-required node does not exist\n");
	}

	if(!priv->link_polling_required)
	{
		link_intr_gpio = of_get_property(np, "qcom,link-intr-gpio", &len);
		if (!link_intr_gpio )
		{
			SSDK_ERROR("cannot find qcom,link-intr-gpio node\n");
			return -1;
		}
#ifndef BOARD_MDM_LE
...
static int ssdk_switch_register(a_uint32_t dev_id)
{
	...
	else
	{
		SSDK_INFO("interrupt is selected\n");
#ifdef BOARD_MDM_LE
		priv->interrupt_flag = IRQF_TRIGGER_LOW|IRQF_SHARED;
#else
		...
		struct device_node *switch_node, ssdk_init_cfg *cfg)
{
	...
	if (of_property_read_u32(switch_node, "qcom,switch-cpu-bmp", &cfg->port_cfg.cpu_bmp)
		|| of_property_read_u32(switch_node, "qcom,switch-lan-bmp", &cfg->port_cfg.lan_bmp)
		|| of_property_read_u32(switch_node, "qcom,switch-wan-bmp", &cfg->port_cfg.wan_bmp)) {
		...
		ssdk_dt_cfg *ssdk_dt_priv)
{
	...
	if (of_property_read_string(switch_node, "qcom,switch-access-mode", (const char **)&ssdk_dt_priv->reg_access_mode)) {
		SSDK_ERROR("%s: error reading device node properties for qcom,switch-access-mode\n", switch_node->name);
		return SW_BAD_PARAM;
	}
...
static sw_error_t ssdk_dt_qca808x_default(ssdk_init_cfg *cfg, a_uint32_t num, a_uint32_t *dev_idp)
{
	a_uint32_t dev_id = 0;
	ssdk_dt_cfg *ssdk_dt_priv = ssdk_dt_global.ssdk_dt_switch_nodes[dev_id];

	ssdk_dt_priv->device_id = dev_id;
	ssdk_dt_priv->ess_switch_flag = A_TRUE;
	ssdk_dt_priv->of_node = NULL;
	ssdk_dt_priv->ess_clk= ERR_PTR(-ENOENT);
	ssdk_dt_priv->cmnblk_clk = ERR_PTR(-ENOENT);
	ssdk_dt_priv->switch_reg_access_mode = HSL_REG_MDIO;
	qca_ssdk_phy_address_set(dev_id, 5, 28);
	ssdk_dt_priv->port_cfg.cpu_bmp = 0x1;
	ssdk_dt_priv->port_cfg.lan_bmp = 0x3e;
	ssdk_dt_priv->port_cfg.wan_bmp = 0x40;
	qca_ssdk_port_bmp_set(dev_id, ssdk_dt_priv->port_cfg.lan_bmp|ssdk_dt_priv->port_cfg.wan_bmp);
	*dev_idp = dev_id;

	SSDK_INFO("ssdk_dt_qca808x_default done\n");
	return SW_OK;
}
#endif
...
static struct mutex port_link_lock;
static ssdk_port_status port_link_status[AR8327_NUM_PORTS] = {0};
extern struct kobject *ssdk_sys;
static int ssdk_port_link_event_send(struct notifier_block *unused,
				unsigned long event, void *ptr)
{
	ssdk_port_status *port_status = (ssdk_port_status *) ptr;

	mutex_lock(&port_link_lock);
	port_link_status[port_status->port_id] = *port_status;
	mutex_unlock(&port_link_lock);

	if (ssdk_sys)
		sysfs_notify(ssdk_sys, NULL, "port_link");
	return 0;
}

void ssdk_port_link_event_receive(ssdk_port_status *ports_status[])
{
	mutex_lock(&port_link_lock);
	memcpy(ports_status, port_link_status, sizeof(port_link_status));
	mutex_unlock(&port_link_lock);

	return;
}

static struct notifier_block ssdk_port_link_notifier =
			{ .notifier_call = ssdk_port_link_event_send };
...
#ifdef IN_QCA808X_PHY
			if(SW_OK != ssdk_dt_qca808x_default(&cfg, num, &dev_id))
#endif
				continue;
		}
		#endif
...
#ifdef BOARD_MDM_LE
	mutex_init(&port_link_lock);
	ssdk_port_link_notify_register(&ssdk_port_link_notifier);
#endif
...
#ifdef BOARD_MDM_LE
	ssdk_port_link_notify_unregister(&ssdk_port_link_notifier);
#endif
