...
static int qca808x_dev_addr_init(a_uint32_t dev_id, a_uint32_t phy_addr,
								struct net_device *dev)
{
	...
	aos_mem_copy(mac.uc, (a_uint8_t *)dev->dev_addr, sizeof(mac.uc));
	qca808x_phy_set_magic_frame_mac(dev_id, phy_addr, &mac);

	SSDK_INFO("%s %02x-%02x-%02x-%02x-%02x-%02x\n", dev->name, \
		(a_uint32_t)mac.uc[0], (a_uint32_t)mac.uc[1], (a_uint32_t)mac.uc[2],\
		(a_uint32_t)mac.uc[3], (a_uint32_t)mac.uc[4], (a_uint32_t)mac.uc[5]);
...
static int qca808x_set_wol(struct phy_device *phydev, struct ethtool_wolinfo *wol)
{
	...
	enable = (wol->wolopts & WAKE_PHY) ? A_TRUE : A_FALSE;
	if(qca808x_phy_set_wol_status(pdata->dev_id, pdata->phy_addr, enable) != SW_OK)
		return -EPERM;
...
static a_uint32_t qca808x_negtiation_cap_get(struct phy_device *phydev)
{
	...
	if (linkmode_test_bit(ETHTOOL_LINK_MODE_2500baseT_Full_BIT, advertising) ||
		linkmode_test_bit(ETHTOOL_LINK_MODE_2500baseX_Full_BIT, advertising)) {
		autoneg |= FAL_PHY_ADV_2500T_FD;
	}
...
#ifdef NAPA_DEBUG_IRQ
	if (phydev->mdio.addr == 28) {
		phydev->irq = gpio_to_irq (gpio);
		irq_set_irq_type(phydev->irq, IRQ_TYPE_EDGE_FALLING);
		printk("qca808x_phy_probe enable irq:%d\n", phydev->irq);
	}
#endif
...
#endif
	printk("qca808x_phy_driver_register done\n");
	return ret;
}
...
void qca808x_phy_driver_unregister(void)
{
	...
	printk("phy_driver_unregister done\n");
}
