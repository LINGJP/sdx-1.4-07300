...
/*	sw_error_t rv;
	...
	return rv;
*/
	qca808x_phy_reg_write(dev_id, phy_id, QCA808X_MMD_CTRL_REG, mmd_num);
	qca808x_phy_reg_write(dev_id, phy_id, QCA808X_MMD_DATA_REG, reg_id);
	qca808x_phy_reg_write(dev_id, phy_id, QCA808X_MMD_CTRL_REG,
	     0x4000 | mmd_num);
	qca808x_phy_reg_write(dev_id, phy_id, QCA808X_MMD_DATA_REG, reg_val);
...
*/
	...
	return qca808x_phy_reg_read(dev_id, phy_id, QCA808X_MMD_DATA_REG);
}
...
qca808x_phy_hw_init(a_uint32_t dev_id,  a_uint32_t port_bmp)
{
	...
	for (port_id = SSDK_PHYSICAL_PORT0; port_id < SW_MAX_NR_PORT; port_id ++)
	{
		if (port_bmp & (0x1 << port_id))
		{
			...
			/*added for workaround of qca_ar8216_mii_read*/
			rv = qca808x_phy_enable_autoneg(dev_id, phy_addr);
			SW_RTN_ON_ERROR(rv);
		}
	}

	printk("qca808x_phy_hw_init done\n");
	return rv;
}
