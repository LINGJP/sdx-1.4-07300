...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 9, 65))
...
napt_ct_aging_is_enable(uint32_t ct_addr)
{
	...
	hlist_for_each_entry_safe(exp, next, &help->expectations, lnode) {
	    return timer_pending(&exp->timeout);
	}
...
napt_ct_aging_enable(uint32_t ct_addr)
{
    ...
    if ((l3num == AF_INET) && (protonum == IPPROTO_TCP))
    {
        ...
        {
            expires = jiffies+(5*24*60*60*HZ);
        }
    }

	help = nfct_help(ct);
	hlist_for_each_entry_safe(exp, next, &help->expectations, lnode) {
		exp->timeout.expires = expires;
	}
#else
